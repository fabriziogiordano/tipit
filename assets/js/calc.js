...
function addResults(resultArray) {
    ...
    for (let i = 0; i < resultArray.length; i++) {
        ...
        const tipZoomFn = `tipZoom('${formatDecimal(tipPercent, 1)}', '${formatDecimal(resultArray[i].tipAmount, 2)}', '${formatDecimal(resultArray[i].totalAmount, 2)}')`;
...
function tipZoom(tipPercent, tipAmount, totalAmount) {
    ...
    $('#tipFullScreenContainer').innerHTML = `
        <div>
            <span>Tip %</span>
            <em style="color: ${getColorForPercent(tipPercent)}">${tipPercent}%</em>
        </div>
        <div>
